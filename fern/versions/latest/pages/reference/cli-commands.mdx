...
By default, `gym eval compare` also runs a paired statistical significance test on every key metric with per-task pairing data (the same test [`gym eval stat-test`](#gym-eval-stat-test) runs standalone) and writes it to a separate `statistical_tests/` subdirectory next to the report 
...
| `--margin DELTA[,DELTA...]` | Tolerance delta(s) for `--alternative`, e.g. `0.01` for 1pp (default `0`). One value applying to every metric, or one per `--metric` in the same order. Ignored when `--alternative` is `two-sided`. |
| `--alternative` | Which hypothesis to test (default `two-sided`). See [`gym eval stat-test`](#gym-eval-stat-test). |
...
`--alternative` chooses the hypothesis, and `--margin` sets the tolerance it is measured against:

- **`two-sided`** (default): "did anything change at all" — for repeatability checks (same benchmark/model run twice). `--margin` is ignored.
- **`candidate-not-worse`**: "the candidate is not worse than `--margin` allows" — for regression checks with a tolerance (e.g. for quantization: is accuracy within 1pp of the baseline).
- **`candidate-not-better`**: the reverse, for confirming an intended improvement is real.

`--margin` defaults to `0`, so a one-sided test without one is the plain "is the candidate worse/better at all" question.
...
| `--test NAME` | Which statistical test to run. `paired` (the default) is the only one today; `--metric`, `--margin` and `--alternative` below belong to it. |
...
# Quantization: is the candidate not meaningfully worse than a 1pp margin?
gym eval stat-test \
    --baseline runs/bf16/rollouts.jsonl \
    --candidates runs/nvfp4/rollouts.jsonl \
    --metric reward --margin 0.01 --alternative candidate-not-worse
```
